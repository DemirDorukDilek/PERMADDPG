...
class World(BaseWorld):
    ...
    def duz_reset(self):
        ...
        self.agents[0].reset()
        self.agents[0].pos = l + np.array([2.0*self.agents[0].radius,0],dtype=np.float32)
        self.agents[1].reset()
        self.agents[1].pos = l + np.array([-2.0*self.agents[1].radius,0],dtype=np.float32)
        self.agents[2].reset()
        self.agents[2].pos = l + np.array([0,2.0*self.agents[2].radius],dtype=np.float32)
        # self.agents[3].reset()
        # self.agents[3].pos = l + np.array([0,-1.5*self.agents[3].radius],dtype=np.float32)
        for agent in self.agents:
            agent.dist = np.linalg.norm(agent.pos-self.goal.pos)
            invalid.append((agent.pos,agent.radius))
